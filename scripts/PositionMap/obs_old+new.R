...
WD.obs$ch<-chron(dates=sprintf("%04d/%02d/%02d",WD.obs$Year,WD.obs$Month,WD.obs$Day),
                ...
                format=c(dates='y/m/d',times='h:m:s'))
WD.obs.old<-read.csv('../interpolate_positions/interpolated.csv.old')
WD.obs.old$Year<-1894 #as.integer(substr(WD.obs$V2,1,4))
WD.obs.old$Month<-as.integer(substr(WD.obs$V2,6,7))
WD.obs.old$Day<-as.integer(substr(WD.obs$V2,9,10))
WD.obs.old$Hour<-as.integer(substr(WD.obs$V2,12,13))
WD.obs.old$ch<-chron(dates=sprintf("%04d/%02d/%02d",WD.obs$Year,WD.obs$Month,WD.obs$Day),
                times=sprintf("%02d:00:00",WD.obs$Hour),
                format=c(dates='y/m/d',times='h:m:s'))
...
WD.get.obs.old<-function(year,month,day,hour) {
   o.date<-chron(dates=sprintf("%04d/%02d/%02d",year,month,day),
                times=sprintf("%02d:00:00",hour),
                format=c(dates='y/m/d',times='h:m:s'))
    w<-which(WD.obs.old$ch>=o.date-0.5 & WD.obs$ch<=o.date+0.5)
    if(length(w)==0) return(NULL)
    s.o<-WD.obs.old[w,]
    s.o$Latitude<-s.o$V10
    s.o$Longitude<-s.o$V11
...
plot.hour<-function(l.count) {    
    ...
    if (Options.local$show.obs) {
        ...
        Options.local<-WeatherMap.set.option(Options,'obs.colour',rgb(255,0,0,255,
                                                       maxColorValue=255))
        if (is.null(obs.old)) WeatherMap.draw.obs(obs.old, Options.local)
    }
    if (Options$label != "") {
